...
void enterDirectory(std::stack<std::string>& pathHistory,
                    ...
                    std::vector<std::string>& contents, int& selected) {
    ...
    const bool valid_selection =
        ! cache.contents.empty() && (selected >= 0) &&
    ...
    if (! fs::exists(fullPath) ||
        ! FileBrowser::isDirectory(fullPath.string())) {
    ...
    new_cache.contents =
        FileBrowser::getDirectoryContents(fullPath.string()); // 强制刷新
