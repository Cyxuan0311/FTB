...
std::string GetFolderIcon(const std::filesystem::path& path) {
    ...
    // Check exact folder name map FIRST (includes hidden dirs like .git, .vscode, .config)
    if (auto it = kFolderIcons.find(name); it != kFolderIcons.end()) {
        return it->second;
    }
    // Hidden dirs without a specific entry get lock icon
