...
std::vector<std::pair<std::string, std::string>> KeyBindings::GetCommandList() const {
    ...
    list.push_back({"xlsx / xls",          "Toggle XLSX preview"});
    list.push_back({"media / video / mp4 / gif", "Toggle media preview"});
    list.push_back({"timg / play",         "Play media fullscreen"});
    list.push_back({"pdf / hygg",          "Toggle PDF preview"});
    list.push_back({"doc / docx / pandoc", "Toggle DOC/DOCX preview"});
    list.push_back({"hex / xxd",            "Toggle hex preview"});
