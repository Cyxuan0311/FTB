...
// ---- 标签栏风格选项 ----
const std::vector<std::string> kTabBarStyles = {
    "modern", "classic", "rounded", "underline", "elegant", "minimal"
};
...
// ---- 标签栏风格预览 ----
Element TabBarPreview(const std::string& style) {
    Color main_bg        = TC("main_bg");
    ...
    auto make_tab = [&](const std::string& name, bool active) -> Element {
        if (!active) {
            if (style == "minimal") {
                return hbox({
                    text("   "),
                    text(name) | color(tab_inactive_fg) | bgcolor(main_bg),
                    text(" "),
                });
            }
            return text(" " + name + " ") | color(tab_inactive_fg) | bgcolor(main_bg);
        }

        if (style == "classic") {
            return text(" " + name + " ") | bgcolor(tab_active_bg) | color(tab_active_fg) | bold;
        }
        if (style == "rounded") {
            return hbox({
                text(PowerlineChars::LeftRound) | color(tab_active_bg) | bgcolor(main_bg),
                text(" " + name + " ") | color(tab_active_fg) | bgcolor(tab_active_bg) | bold,
                text(PowerlineChars::RightRound) | color(tab_active_bg) | bgcolor(main_bg),
            });
        }
        if (style == "underline") {
            return text(" " + name + " ") | color(tab_active_fg) | bgcolor(main_bg) | bold | underlined;
        }
        if (style == "elegant") {
            return hbox({
        ...
        if (style == "minimal") {
            return hbox({
                text(" \u25cf ") | color(tab_active_bg) | bgcolor(main_bg),
                text(name) | color(tab_active_fg) | bgcolor(main_bg) | bold,
    ...
    Elements tabs;
    tabs.push_back(make_tab("documents", true));
    tabs.push_back(text(" \u2502 ") | color(border_col) | bgcolor(main_bg));
    tabs.push_back(make_tab("downloads", false));
    tabs.push_back(text(" \u2502 ") | color(border_col) | bgcolor(main_bg));
    tabs.push_back(make_tab("projects", false));

    return vbox({
        hbox(tabs) | bgcolor(main_bg),
        separator() | color(border_col)
    }) | size(WIDTH, EQUAL, 42);
}
...
Element RenderUIStylePanel(MainState& state, int /*tw*/, int /*th*/) {
    ...
    int tab_idx = FindIndex(kTabBarStyles, cfg.ui.tab_bar_style);
    ...
    std::string current_tab = kTabBarStyles[tab_idx];
    ...
    // Tab Bar section
    rows.push_back(
        text("  Tab Bar Style") | color(TC("path")) | bold | underlined
    );
    {
        std::string indicator = (state.panel_selected == 3) ? " > " : "   ";
        rows.push_back(
            hbox({
                text(indicator) | color(TC("indicator")),
                text(current_tab) | color(TC("main_fg")) | bold,
                text("  "),
                TabBarPreview(current_tab)
            })
        );
    }
...
bool HandleUIStyleEvent(MainState& state, const Event& event) {
    ...
    if (event == Event::Escape) {
        ...
        cfg.ui.tab_bar_style = state.panel_orig_tab_bar_style;
    ...
    if (event == Event::ArrowDown || event == Event::Character('j')) {
        if (state.panel_selected < 3) state.panel_selected++;
        return true;
    }
    ...
    if (event == Event::ArrowRight || event == Event::Character('l')) {
        ...
        } else if (state.panel_selected == 3) {
            int idx = FindIndex(kTabBarStyles, cfg.ui.tab_bar_style);
            idx = (idx + 1) % kTabBarStyles.size();
            cfg.ui.tab_bar_style = kTabBarStyles[idx];
        }
        return true;
    }
    if (event == Event::ArrowLeft || event == Event::Character('h')) {
        if (state.panel_selected == 0) {
            ...
            int idx = FindIndex(kTabBarStyles, cfg.ui.tab_bar_style);
            idx = (idx - 1 + kTabBarStyles.size()) % kTabBarStyles.size();
            cfg.ui.tab_bar_style = kTabBarStyles[idx];
        }
        return true;
    }
