...
std::vector<DirEntryInfo> getDirectoryEntries(const std::string& path, FTB::SortMode mode) {
    auto entries = getDirectoryEntries(path);
    FTB::SortEntries(entries, mode);
    return entries;
}
...
bool moveToTrash(const std::string & path) {
    ...
    try {
...
#ifdef __APPLE__
        std::string trash_dir = std::string(home) + "/.Trash";
#else
        std::string trash_dir = std::string(home) + "/.local/share/Trash/files";
        std::string info_dir  = std::string(home) + "/.local/share/Trash/info";
        fs::create_directories(trash_dir);
        fs::create_directories(info_dir);
#endif

        std::string filename = fs::path(path).filename().string();
        std::string dest = (fs::path(trash_dir) / filename).string();

        int counter = 1;
        while (fs::exists(dest)) {
            std::string new_name = filename + "." + std::to_string(counter);
            dest = (fs::path(trash_dir) / new_name).string();
            counter++;
        }
        ...
        std::string info_path = (fs::path(info_dir) / (fs::path(dest).filename().string() + ".trashinfo")).string();
        std::ofstream info_file(info_path);
        if (info_file) {
            info_file << "[Trash Info]\n";
            info_file << "Path=" << orig_path << "\n";
            info_file << "DeletionDate=" << ss.str() << "\n";
        }
#endif
