...
static std::vector<ArchiveEntry> ListWithCommands(const std::string& filePath,
    ...
    // Tier 1: 直接匹配已知格式
    if (ext == ".zip") {
        auto out = RunCommand("unzip -l " + escaped + " 2>/dev/null");
        if (!out.empty()) {
    ...
    if (ext == ".tar" || name_lower.find(".tar.") != std::string::npos ||
        ...
        auto out = RunCommand("tar -tvf " + escaped + " 2>/dev/null");
        if (!out.empty()) {
    ...
    if (ext == ".7z" || ext == ".rar" || ext == ".cab") {
        auto out = RunCommand("7z l " + escaped + " 2>/dev/null");
        if (!out.empty()) {
    ...
    if (ext == ".iso") {
        auto out = RunCommand("isoinfo -l " + escaped + " 2>/dev/null");
        if (!out.empty()) {
