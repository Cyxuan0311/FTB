...
int main() {
    ...
    auto key_info = frame(
        vbox({
            ...
            text("Ctrl+p键 指定预览文件的内容") | color(Color::GrayDark) | size(HEIGHT, LESS_THAN, 1),
            text("Ctrl+e键 进入Vim-Like编辑模式") | color(Color::GrayDark) | size(HEIGHT, LESS_THAN, 1),
    ...
    auto renderer = Renderer(component, [&] {
        ...
        for (size_t i = 0; i < filteredContents.size(); ++i) {
            ...
            if (pos != std::string::npos && !searchQuery.empty()) {
                ...
                highlighted.push_back(text(itemText.substr(pos, searchQuery.size())) | color(Color::GrayLight));
                if (pos + searchQuery.size() < itemText.size())
                    highlighted.push_back(text(itemText.substr(pos + searchQuery.size())));
                elements.push_back(
                    hbox({text(selected == (int)i ? "→ " : "  "), text(icon), hbox(highlighted) | bold | text_color | (selected == (int)i ? underlined : nothing), filler()})
                    | borderHeavy | bg_style | size(WIDTH, LESS_THAN, 50)
                );
            } else {
                elements.push_back(
                    hbox({text(selected == (int)i ? "→ " : "  "), text(icon), text(itemText) | bold | text_color | (selected == (int)i ? underlined : nothing), filler()})
                    | borderHeavy | bg_style | size(WIDTH, LESS_THAN, 50)
                );
            }
        }
        ...
        return vbox({
            hbox({
                vbox({
                    hbox({text("FTB") | bold | borderDouble | bgcolor(Color::BlueLight) | size(WIDTH, LESS_THAN, 5),
                ...
                key_info,
                vbox({ text(time_str) | color(Color::GrayDark), text(loadingIndicator) | color(Color::Green) })
  ...
  auto final_component = CatchEvent(splitted, [&](Event event) {
    ...
    if (UIManager::handleEvents(event, directoryHistory, currentPath,allContents, filteredContents, selected,searchQuery, screen, refresh_ui,vim_mode_active
        , vimEditor))
      return true;
    return false;
  });
