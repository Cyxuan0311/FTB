...
auto calculateSizes = [](int& selected, std::string& currentPath,
    ...
    if (selected != last_selected || currentPath != last_path) {
        ...
        if (size_future.valid() && size_future.wait_for(std::chrono::seconds(
        ...
        // 检查当前目录是否为空，避免重复计算
        {
            ...
            if (! cache.valid) {
                cache.contents =
                    FileBrowser::getDirectoryContents(local_currentPath);
        ...
        // 异步计算文件大小和比例
        size_future =
            std::async(std::launch::async, [local_selected, local_currentPath,
                                            &total_folder_size, &size_ratio,
                                            &selected_size]() {
                std::lock_guard<std::mutex> lock(cache_mutex);
                auto& cache = dir_cache[local_currentPath];
                if (! cache.valid) {
                ...
                for (const auto& item : cache.contents) {
                    std::string fullPath =
                        (fs::path(local_currentPath) / item).string();
                ...
                if (! cache.contents.empty() &&
                    local_selected < static_cast<int>(cache.contents.size())) {
                    uintmax_t size = sizes[local_selected];
                    double ratio =
                        total > 0 ? static_cast<double>(size) / total : 0.0;
...
int main() {
    ...
    auto renderer = Renderer(component, [&] {
        calculateSizes(selected, currentPath, size_future, total_folder_size,
                       size_ratio, selected_size);
        ...
        for (size_t i = 0; i < filteredContents.size(); ++i) {
            ...
            auto text_color = is_dir ? color(Color::RGB(135, 206, 250))
            ...
            if (selected == (int) i) {
                bg_style =
                    bgcolor(Color::GrayLight) | color(Color::Black) | bold;
            }
            ...
            if (pos != std::string::npos && ! searchQuery.empty()) {
                ...
                highlighted.push_back(
                    text(itemText.substr(pos, searchQuery.length())) |
                ...
                elements.push_back(
                    hbox({text(selected == (int) i ? "→ " : "  "), text(icon),
                ...
                elements.push_back(
                    hbox({text(selected == (int) i ? "→ " : "  "), text(icon),
                          text(itemText) | bold | text_color | text_color |
                              (selected == (int) i ? underlined : nothing),
                          filler()}) |
                    border | bg_style | size(WIDTH, LESS_THAN, 50));
            }
        ...
        for (size_t start = 0; start < elements.size();
             start += max_items_per_column) {
            auto end = std::min(start + max_items_per_column, elements.size());
            columns.emplace_back(elements.begin() + start,
                                 elements.begin() + end);
        }
        ...
        std::string loadingIndicator =
            loadingFrames[loadingIndex % loadingFrames.size()];
        loadingIndex = (loadingIndex + 1) % (loadingFrames.size() * 2);
        std::ostringstream ratio_stream;
        ratio_stream << std::fixed << std::setprecision(2)
                     << (size_ratio.load() * 100);

        return vbox(Elements{
            hbox({text("FTB") | bold | borderDouble |
                      bgcolor(Color::BlueLight) | size(WIDTH, LESS_THAN, 5) |
                      size(HEIGHT, LESS_THAN, 1),
                  filler() | size(WIDTH, EQUAL, 2),
                  text("🤖当前路径: " + displayPath) | bold | border |
                      color(Color::Pink1) | size(HEIGHT, LESS_THAN, 1),
                  filler(),
                  vbox(Elements{
                      ...
                      hbox({text("["),
                            gauge(size_ratio.load()) | flex |
                                color(Color::Green) | size(WIDTH, EQUAL, 20),
                            text("]")}),
                  ...
                  vbox(Elements{
                      ...
                      text("↑/↓ 导航文件列表") | color(Color::GrayDark) |
                          size(HEIGHT, LESS_THAN, 1),
                      text("Enter 进入目录") | color(Color::GrayDark) |
                          size(HEIGHT, LESS_THAN, 1),
                      text("Backspace 返回上级") | color(Color::GrayDark) |
                          size(HEIGHT, LESS_THAN, 1),
                      text("^键 新建文件夹/&键 新建文件") |
                          color(Color::GrayDark) | size(HEIGHT, LESS_THAN, 1),
                      text("~键 删除文件夹或文件") | color(Color::GrayDark) |
                          size(HEIGHT, LESS_THAN, 1),
                      text("ESC 退出程序") | color(Color::Red3) |
                          size(HEIGHT, LESS_THAN, 1)}) |
                      border | color(Color::Purple3) |
                      size(WIDTH, LESS_THAN, 30),

                  vbox(Elements{text(time_str) | color(Color::GrayDark),
                                text(loadingIndicator) | color(Color::Green)}) |
                      border |
                      color(Color::Purple3)}) /*| size(HEIGHT, EQUAL, 9)*/,
            searchInput->Render() | border | color(Color::Magenta) |
                size(WIDTH, LESS_THAN, 100),
            hbox(column_boxes) | color(Color::Blue) | frame | border |
    ...
    auto final_component = CatchEvent(renderer, [&](ftxui::Event event) {
        return handleEvents(event, pathHistory, currentPath, allContents,
                            filteredContents, selected, searchQuery, screen,
                            refresh_ui);
    });
